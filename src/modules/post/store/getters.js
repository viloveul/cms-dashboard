let groupingItems = (items) => {
  ...
  for (let i = 0; i < items.length; i++) {
    if (typeof tmpItems[items[i].parent_id] === 'undefined') {
      tmpItems[items[i].parent_id] = []
    }
    tmpItems[items[i].parent_id].push(items[i])
  }
  return tmpItems
}
...
let recursiveMapItems = (results, items, parentId, prefix) => {
  if (items[parentId] !== undefined) {
    for (let a = 0; a < items[parentId].length; a++) {
      ...
      item.display = prefix + items[parentId][a].title
      results.push(item)
